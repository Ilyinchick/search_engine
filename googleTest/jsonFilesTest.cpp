...
TEST(jsonFiles, configExist) {
    std::ifstream file("../executable/manage/config.json", std::ios::in);
    EXPECT_TRUE(file.is_open());
}

TEST (jsonFiles, configNotEmpty) {
    std::ifstream file("../executable/manage/config.json");
    nlohmann::json jfile;
    file >> jfile;
...
TEST (jsonFiles, requestsFileExist) {
    std::ifstream file("../executable/manage/requests.json");
    EXPECT_TRUE(file.is_open());
}
